...
if(!$space['self'] && $_SGLOBAL['supe_uid']) {		
	...
	if(empty($visitor['dateline'])) {
		...
		if($_SGLOBAL['timestamp'] - $visitor['dateline'] >= 300) {
			updatetable('visitor', array('dateline'=>$_SGLOBAL['timestamp'], 'vusername'=>$is_anonymous ? '' : $_SGLOBAL['supe_username'],'vname'=>$is_anonymous ? '' : $vname), array('uid'=>$space['uid'], 'vuid'=>$_SGLOBAL['supe_uid']));
		}
